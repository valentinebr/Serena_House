        ...
        foreach ($tinyHouse as $t) { ?>
            <tr>
                ...
                <?php if ($recurrence[$t->nom_tiny] > 1) {
                    if ($j == 0) {
                        $j++; ?>
                        <td rowspan="<?php echo $recurrence[$t->nom_tiny] ?>"><a href="#"
                                                                                 onclick="modifierTiny('modifier', <?php echo htmlspecialchars(json_encode($t)) . ',' . htmlspecialchars(json_encode($tinyHouse)) ?>)">Modifier</a>
                        </td>
                        <td rowspan="<?php echo $recurrence[$t->nom_tiny] ?>"><a
                                    href="?ctrl=TinyHouse&amp;action=deleteTinyHouse&amp;id=<?php echo $t->id_tiny ?>">Supprimer</a>
                        </td>
                    <?php } else {
                        $j++;
                ...
                <?php } else {
                    ...
                    <td>
                        <a href="?ctrl=TinyHouse&amp;action=deleteTinyHouse&amp;id=<?php echo $t->id_tiny ?>">Supprimer</a>
                    </td>
                <?php } ?>
            </tr>
        <?php }
