...
if( ! class_exists( 'Avia_Custom_Pages' ) )
{
	class Avia_Custom_Pages 
	{
		...
		public function handler_hide_special_pages( WP_Query $query )
		{
			...
			/**
			 * Fixes issue which puts page ID to 'post__not_in' even for query, which has it at 'post__in' (query the page object).
			 * Some plugins (like User Role Editor Pro) respects 'post__not_in' with higher priority than WP_Query does and remove it.
			 * https://kriesi.at/support/topic/user-role-editor-bug/#post-1179446
			 * 
			 * @since 4.7.2.1
			 */
			$in = $query->get( 'post__in' );
			
			foreach( $not_in as $id_not_in ) 
			{
				if ( ! in_array( $id_not_in, $pages ) ) 
				{
					continue;
				}
				
				foreach( $in as $index => $id_in ) 
				{
					if ( $id_not_in == $id_in ) 
					{
						unset( $not_in[ $index ] );
