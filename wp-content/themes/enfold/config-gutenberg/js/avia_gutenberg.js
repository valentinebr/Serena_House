...
(function($)
{
	$.AviaBuilderGutenberg = function()
	{
		...
		{
			this.body_container.on( 'AviaBuilder_after_set_up', $.proxy( this.set_up, this ));
		}
		else
		{
			this.set_up();
	...
	$.AviaBuilderGutenberg.prototype = {
		...
		copy_layout_switch_button: function()
		{
			...
			new_button.on( 'click', $.proxy( this.switch_layout_mode_gutenberg_btn, this ));
		...
		attach_gutenberg_title_event_handler: function()
		{
			...
			title.on( 'keyup', $.proxy( this.gutenberg_title_keyup, this ) );
		},
		
		attach_edit_permalink_event_handler: function()
		{
			this.body_container.find('.av-edit-alb-permalink').on( 'click', $.proxy( this.alb_edit_permalink, this ));
		},
		
		attach_alb_permalink_href_handler: function()
		{
			this.body_container.find('.avia_meta_box_visual_editor #edit-slug-box #sample-permalink > a').on('click', $.proxy( this.alb_permalink_href_handler, this ));
		},
		
		attach_handlers: function()
		{
			this.body_container.on( 'AviaBuilder_after_switch_layout_mode', $.proxy( this.layout_mode_changed, this ));
			this.body_container.find('.avia_meta_box_visual_editor #titlewrap input').on( 'keyup', $.proxy( this.alb_title_key_up, this ) );
			this.attach_gutenberg_title_event_handler();
			this.aviaBuilder.secureContent.on('av_secureContent_update change', $.proxy( this.alb_secureContent_changed, this ));
...
//				var sav = editor.savePost();
		...
		monitor_save_infos: function()
		{
			if( null == this.auto_save_observer )
			{
				...
				this.auto_save_observer = new MutationObserver( $.proxy( this.check_for_is_saving, this ) );
	...
	$(document).ready(function () 
	{
    	...
    	{
			console.log('Class AviaBuilderGutenberg loaded');
